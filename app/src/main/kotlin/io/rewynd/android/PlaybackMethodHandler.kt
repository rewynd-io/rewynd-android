...
object PlaybackMethodHandler {
    suspend fun next(client: RewyndClient, playerMedia: PlayerMedia): PlayerMedia? =
        when (playerMedia) {
            is PlayerMedia.Episode -> EpisodePlaybackMethodHandler.next(client, playerMedia)
        }

    suspend fun prev(client: RewyndClient, playerMedia: PlayerMedia): PlayerMedia? =
        when (playerMedia) {
    ...
    suspend fun next(
        ...
        client.getNextEpisode(playerMedia.info.id).body().let {
            val progress = client.getUserProgress(it.id).body()
                .run { (if (percent > 0.9) 0.0 else percent) }
            PlayerMedia.Episode(
                playbackMethod = playerMedia.playbackMethod,
                info = it,
                runTime = it.runTime.seconds,
                startOffset = (progress * it.runTime).seconds
            )
        }
    } catch (e: Exception) {
